...
class LunrIndexConfig(AppConfig):
    ...
    def ready(self):
        ...
        if (
            getattr(settings, "LUNR_INDEX_BACKEND", None)
            in LunrBackendValue.VALID_BACKENDS
        ):
            REBUILD_INTERVAL = settings.LUNR_INDEX_REBUILD_INTERVAL
            REBUILD_TIMEOUT = settings.LUNR_INDEX_REBUILD_TIMEOUT
            self.signal_processor = LunrIndexSignalProcessor(
                REBUILD_INTERVAL, REBUILD_TIMEOUT
            )
