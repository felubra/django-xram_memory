...
class NewspaperArticleParser(BasicInfoPluginBase):
    ...
    def parse(cls, url, html=None):
        try:
            ...
            result = cls.clean(
                {
                    ...
                    "keywords": cls.extract_taxonomy(
                        getattr(newspaper_article, "keywords", []), cls.KEYWORDS_REGEX
                    ),
                    "subjects": cls.extract_taxonomy(
                        getattr(newspaper_article, "keywords", []), cls.SUBJECTS_REGEX
                    ),
                }
            )
            ...
            raise (
                ValueError(
                    "Falha ao construir o dicionário com as informações básicas da notícia: {}.".format(
                        str(err)
                    )
                )
            )
...
class Goose3NewspaperArticleParser(BasicInfoPluginBase):
    ...
    def parse(cls, url, html=None):
        ...
        try:
            ...
            meta_keywords = [
                keyword.strip()
                for keyword in getattr(goose_article, "meta_keywords", "").split(",")
            ]
            keywords = list(set(getattr(goose_article, "keywords", []) + meta_keywords))
            result = cls.clean(
                {
                    "title": getattr(goose_article, "title", ""),
                    "image": (
                        goose_article.top_image.src
                        if isinstance(getattr(goose_article, "top_image", None), Image)
                        else ""
                    ),
