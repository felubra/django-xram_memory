...
def test_localsearch_remote_elastic_lunr(settings, mocker):
    ...
    mocker.patch(
        "xram_memory.lunr_index.lib.index_builders.RemoteElasticLunrIndexBuilder._get_client",
        return_value=mocked_client,
    )
    mocker.patch(
        "xram_memory.lunr_index.signals.celery_is_avaliable", return_value=False
    )
...
def test_localsearch_lunr_py(settings, mocker):
    ...
    try:
        (
            fd,
            file_path,
        ) = tempfile.mkstemp(dir=settings.MEDIA_ROOT)
